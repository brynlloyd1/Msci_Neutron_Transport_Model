...
class Moderation_Experiment_DTPDXP:
    def __init__(self, n_runs, time=800):
        ...
        # self.H_fraction_hotspot = np.linspace(0,1, n_runs)
    ...
    def run_experiment(self):
        ...
        for i in range(self.n_runs):
            ...
            vary_hotspot_params = np.array([[1e30, self.fuel_fraction_hotspot[i], self.fuel_fraction_hotspot[i], self.H_fraction_hotspot[i], 0, 450e-6],
                                            [1e30, 0, 0, 0, 1, 50e-6]])
            
            vary_ablator_params = np.array([[1e30, 0.5, 0.5, 0, 0, 450e-6],
                                            [1e30, 0, 0, self.H_fraction_ablator[i], self.fuel_fraction_ablator[i], 50e-6]])
                                            # [1e30, 0, self.fuel_fraction_ablator[i], self.H_fraction_ablator[i], 0, 50e-6]])
