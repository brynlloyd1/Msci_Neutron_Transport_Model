...
class Moderation_Experiment_DTCRYO:
    def __init__(self, n_runs, time=800):

        self.En = np.geomspace(1e-5, 16, 500)[::-1]
        self.default_params = np.array([[1e30, 0.5, 0.5, 0, 0, 35e-6],
        ...
        self.fuel_fraction_hotspot = (1 - self.H_fraction_hotspot)/2
        ...
        self.fuel_fraction_shell = (1 - self.H_fraction_shell)/2
    ...
    def run_experiment(self):
        ...
        for i in range(self.n_runs):
            ...
            vary_hotspot_params = np.array([[self.default_params[0,0], self.fuel_fraction_hotspot[i], self.fuel_fraction_hotspot[i], self.H_fraction_hotspot[i], 0, 35e-6],
                                            [self.default_params[1,0], 0.5, 0.5, 0, 0, 1e-5],
                                            [self.default_params[2,0], 0, 0, 0, 1, 1e-5]])

            vary_shell_params = np.array([[self.default_params[0,0], 0.5, 0.5, 0, 0, 35e-6],
                                          [self.default_params[1,0], self.fuel_fraction_shell[i], self.fuel_fraction_shell[i], self.H_fraction_shell[i], 0, 1e-5],
                                          [self.default_params[2,0], 0, 0, 0, 1, 1e-5]])

            add_ablator_params = np.array([[self.default_params[0,0], 0.5, 0.5, 0, 0, self.default_params[0,5]],
                                           [self.default_params[1,0], 0.5, 0.5, 0, 0, self.default_params[1,5]],
...
# e_test = Moderation_Experiment_DTCRYO(n_runs=5, time=320)
# e_test.run_experiment()